<?php
class ModelCatalogVDIOption extends Model {
	public function addOption($data) {
		$this->event->trigger('pre.admin.add.option', $data);

		$this->db->query("INSERT INTO `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', vendor_id = '" . (int)$this->user->getVP() . "', sort_order = '" . (int)$data['sort_order'] . "'");
		...
		$this->event->trigger('post.admin.add.option', $option_id);

		return $option_id;
	}

	public function editOption($option_id, $data) {
		$this->event->trigger('pre.admin.edit.option', $data);

		$this->db->query("UPDATE `" . DB_PREFIX . "option` SET type = '" . $this->db->escape($data['type']) . "', vendor_id = '" . (int)$this->user->getVP() . "', sort_order = '" . (int)$data['sort_order'] . "' WHERE option_id = '" . (int)$option_id . "'");
		...
		$this->event->trigger('post.admin.edit.option', $option_id);
	}

	public function deleteOption($option_id) {
		$this->event->trigger('pre.admin.delete.option', $option_id);
		...
		$this->event->trigger('post.admin.delete.option', $option_id);
	}
	...
	public function getOptions($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "option` o LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE od.language_id = '" . (int)$this->config->get('config_language_id') . "' AND (o.vendor_id = '" . (int)$this->user->getVP() . "' OR o.vendor_id = '0')";
	...
	public function getTotalOptions() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "option` WHERE (vendor_id = '" . (int)$this->user->getVP() . "' OR vendor_id = '0')"); 
		
		return $query->row['total'];
	}
	
	public function ValidateOPUpdate($option_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "option` o WHERE o.option_id = '" . (int)$option_id . "' AND o.vendor_id = '" . (int)$this->user->getVP() . "'");
		return $query->row['total'];
	}
}